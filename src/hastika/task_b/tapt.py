...
def main():
    ...
    ap.add_argument("--model", default="google/muril-base-cased",
                    help="a checkpoint with an MLM head. The abusive-tuned MuRIL is a "
                         ...
                         "lower --lr if you do")
    ap.add_argument("--out", default="artifacts/runs/tapt-muril")
    ap.add_argument("--corpus", nargs="*", default=None,
                    help="sources to adapt on; defaults to the Task B train file plus "
                         ...
                         ".jsonl, or @manifest -- see hastika.task_b.corpora")
    ...
    print(f"\nwrote {out}\nuse it with:  python -m hastika.task_b.train "
          f"--model {args.out} --tag muril_b_tapt",
          flush=True)
