...
from hastika.models.ensemble import ROOT, RUNS, macro_f1
...
def main():
    ...
    ap.add_argument("--run", required=True, help="a 5-fold run tag under artifacts/runs/")
    ...
    if oof.shape != (len(y), len(LABELS)):
        # a Task A run sharing artifacts/runs/; the sweep hands us every directory
    ...
    ids = pd.read_csv(RAW_DATA_DIR / "multiclass_validation_inputs.csv")["id"]
