...
Every run in artifacts/runs/<tag>/ that has both oof_probs.npy and test_probs.npy is
...
run left in artifacts/runs/ would therefore be loaded, compared against the wrong
...
def main():
    ...
    train = pd.read_csv(RAW_DATA_DIR / spec["train"])
    test_df = pd.read_csv(RAW_DATA_DIR / spec["test"])
