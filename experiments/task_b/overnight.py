...
STAGE1 = [
    ("b_tapt",     ["--model", "artifacts/runs/tapt-muril"],   "MLM-adapted MuRIL"),
...
def package(tag, subs, results, kind):
    ...
    if not pred.exists():
        return None
    z = subs / f"{tag}.zip"
    rc = sh([sys.executable, "-m", "hastika.common.submission", "--task", "b",
...
def main():
    ...
    if not a.skip_tapt and not (RUNS / "tapt-muril").exists():
        if budget.fits(1200):
            sh([py, "-u", "-m", "hastika.task_b.tapt", "--out",
                "artifacts/runs/tapt-muril"],
               log=logs / "tapt.log")
        else:
    ...
    for tag, flags, note in arms:
        ...
        t = time.time()
        log = logs / f"{tag}.log"
        rc = sh([py, "-u", "-m", "hastika.task_b.train", "--tag", tag, "--folds", "0",
    ...
    for tag in promote:
        ...
        t = time.time()
        log = logs / f"{full}.log"
        rc = sh([py, "-u", "-m", "hastika.task_b.train", "--tag", full, "--folds", "5",
    ...
    sh([py, "-m", "hastika.models.ensemble", "--task", "b"],
       log=logs / "ensemble_b.log")
    ...
    for d in sorted(RUNS.iterdir()):
        ...
        if tag.endswith("_decoded") or not (d / "oof_probs.npy").exists():
            continue
        dec = f"{tag}_decoded"
        rc = sh([py, "-m", "hastika.task_b.decode", "--run", tag, "--out", dec,
