...
SMALL = ["data/raw/multiclass_train.csv", "data/external/offenseval_kn.csv"]
...
def main():
    ...
    for tag, model, d, v, aux in configs:
        ...
        cmd = [py, "-u", "-m", "hastika.task_b.train", "--tag", f"g_{tag}", "--folds", "0",
    ...
    for tag in ranked:
        ...
        cmd = [py, "-u", "-m", "hastika.task_b.train", "--tag", f"c_{tag}", "--folds", "0",
    ...
    if a.final_full_fit and ranked:
        winner_tag = max(
            ranked,
            key=lambda tag: (res.get(tag + "_multiseed") or res[tag]).get("f1", -1),
        )
        win = res.get(winner_tag + "_multiseed") or res[winner_tag]
        est = (np.median(times) if times else 1250) * 5 * 1.2
        if left() >= est:
            cmd = [py, "-u", "-m", "hastika.task_b.train", "--tag", "g_winner_full", "--folds", "1",
            ...
            if pred.exists():
                sh([py, "-m", "hastika.common.submission", "--task", "b", "--pred", str(pred),
    ...
    for tag in list(res):
        run_tag = f"c_{tag.removesuffix('_multiseed')}" if tag.endswith("_multiseed") else f"g_{tag}"
        p = RUNS / run_tag / "predictions.csv"
        if p.exists():
            sh([py, "-m", "hastika.common.submission", "--task", "b", "--pred", str(p),
