...
from muril import RUNS, ROOT, SPLIT_SEED, N_SPLITS, train_fold
...
def build_trimmed_vocab(tok, max_len, demoji):
    ...
    for name in ["binary_train.csv", "binary_validation_inputs.csv",
        ...
        for t in pd.read_csv(ROOT / "data" / name)["Comment"]:
...
def main():
    ...
    ap.add_argument("--loss", choices=["ce", "focal"], default="ce",
                    help="focal down-weights already-correct rows, aimed at the 17%% with "
                         ...
                         "UNMEASURED on MuRIL -- A/B it with work/sweep_b.sh before adopting")
    ...
    test = pd.read_csv(ROOT / "data" / "multiclass_validation_inputs.csv")
