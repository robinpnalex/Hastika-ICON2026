...
ARMS = [
    ("f_tapt",    ["--model", "work/runs/tapt-muril"],    0.5972, "MLM-adapted MuRIL"),
...
def main():
    ...
    ap.add_argument("--skip-tapt", action="store_true",
                    help="assume work/runs/tapt-muril exists or drop the f_tapt arm")
    a = ap.parse_args()
    ...
    if not a.skip_tapt and not (RUNS / "tapt-muril").exists():
        if left() > 1800:
            sh([sys.executable, "-u", "work/tapt.py", "--out", "work/runs/tapt-muril"],
               log=ROOT / "work" / "tapt.log")
        else:
    ...
    for tag, flags, hold, note in arms:
        ...
        if left() < est:
            ...
            continue
        t = time.time()
        log = ROOT / "work" / f"{tag}.log"
        rc = sh([sys.executable, "-u", "work/muril_b.py", "--tag", tag, "--folds", "1",
