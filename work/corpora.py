...
nothing else. Accepts:
    ...
    data/multiclass_train.csv          a CSV; the text column is auto-detected
    ...
    @work/corpora/big.txt              a manifest: one source per line, # comments
            ...
            if path.name in GATED:
                if not allow_transductive:
                    sys.exit(f"refusing {path.name}: it carries Task B test comments. "
                             ...
                             f"the POLICY note in work/corpora.py.")
