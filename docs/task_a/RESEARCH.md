...
Copy the whole repo (or just `data/` + `src/`) across, then:
...
```sh
PY=python bash experiments/task_a/run_all.sh          # full sweep + blend + zipped submission
LARGE=0 bash experiments/task_a/run_all.sh            # base models only, ~1.5h
FOLDS=0 EPOCHS=6 bash experiments/task_a/run_all.sh   # quick holdout pass to sanity-check the box
```

Output lands in `artifacts/runs/<tag>/` and the final upload is
`artifacts/runs/ensemble/predictions.zip`.
...
## The MuRIL recipe (`src/hastika/models/muril.py`)
...
```sh
python -m hastika.models.muril --tag smoke --limit 160 --folds 2 --epochs 1 \
    --bs 8 --max-len 32 --threads 4
rm -rf artifacts/runs/smoke        # else ensemble.py will auto-discover it
```
