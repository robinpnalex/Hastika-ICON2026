# Research code reference

Reference for the advanced modeling modules used by both subtasks. Run commands
through `uv` from the repository root. Task B has its operational guide in
[`../task_b/GUIDE.md`](../task_b/GUIDE.md).
...
$V -m hastika.models.baseline_svm --task b --demojize          # floor, 0.5948

# the current recipe, D0_V0_noaux, on every comment and every row
$V -m hastika.task_b.tapt --corpus data/raw/multiclass_train.csv data/external/offenseval_kn.csv \
    --val-frac 0 --min-words 1 --no-dedupe --out artifacts/runs/tapt-d0v0-100   # ~25 min on a T4
$V -m hastika.task_b.train --tag b_d0v0_noaux_full --model artifacts/runs/tapt-d0v0-100 \
    --folds 1 --no-dedupe --aux-weight 0 --seeds 42 43 44 45 46             # ~95 min
$V -m hastika.common.submission --task b --pred artifacts/runs/b_d0v0_noaux_full/predictions.csv

# the same recipe with a local score, as submitted in b_tapt_5f
$V -m hastika.task_b.tapt --out artifacts/runs/tapt-muril
$V -m hastika.task_b.train --tag b_tapt_5f --model artifacts/runs/tapt-muril --folds 5
```
...
Each run writes to `artifacts/runs/<tag>/`: `predictions.csv`, `test_probs.npy`, and
...
cost 3 to 8 points in every cell. Full table in `docs/EXPERIMENTS.md`. The code stays
...
| `split_b.py` | Materializes the fixed 472-row holdout as labelled CSVs under `data/derived/task_b_holdout/`. |

Three levers, usable separately or together:

```sh
# 1. more adaptation data. Tamil + Malayalam measured as no gain in Run 3
$V -m hastika.task_b.tapt --extra 'data/external/*.csv' scraped/comments.txt
$V -m hastika.task_b.tapt --allow-transductive --extra data/raw/binary_train.csv   # rules call

# 2. a tokenizer that fits this register. Measured as 3-8 points WORSE in Run 3
$V -m hastika.task_b.extend_vocab --min-freq 5 --out artifacts/runs/muril-extended
$V -m hastika.task_b.tapt --model artifacts/runs/muril-extended --out artifacts/runs/tapt-extended

# 3. an auxiliary head on the act axis. Measured as no gain in Run 3
$V -m hastika.task_b.train --tag b_aux --aux-weight 0.3
```
